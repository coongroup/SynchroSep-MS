...
def prec_count(df,file):
    ...
    df = df[df['Run']==file]
    prec_count = len(df['Precursor.Id'].unique())
    return prec_count
...
def cum_count(report, run):
    ...
    for rt in rt_bins:
        ...
        cum_count.append(len(run_report[run_report['RT']<=rt]['Precursor.Id'].unique()))
...
def diann_filter(report_path):
    ...
    report = report.reset_index(drop = True)
    report = report.drop_duplicates(subset = ['Run','Precursor.Id'])
    return report
...
def rsd_list(report,files):
    ...
    rsd_vals = []
    for row in report.groupby('Precursor.Id'):
        if len(row[1]['Precursor.Id'])==3:
            rsd_value = rsd_calc(row[1]['Precursor.Normalised'])
...
import numpy as np
fig, (ax1,ax2,ax3) = plt.subplots(3,1,figsize = (3.5,6),layout = 'constrained')
...
ax1.set_ylabel('Detected Precursors',fontsize = 10)
...
#cumulative precursor identifications
...
ax2.legend(frameon = False,labelspacing = 0.13,handletextpad = 0.3,handlelength = 1.5,loc = [0.48,-0.02])
ax2.set_xlim(0,)
ax2.set_ylim(0,)
ax2.set_xlabel('Retention Time (min)')
ax2.set_ylabel('Cumulative Precursors')
...
fig.savefig('C:/Users/nlancaster/OneDrive - UW-Madison/MultiColumnManuscript/ManuscriptDraft/PythonFigures/20250610_Rd07_Precursor_Ids_v4.svg')